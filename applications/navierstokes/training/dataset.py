...
class WaterlilySerial(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cache is not None and filename in self.cache:
            ...
            x = torch.tensor(np.array(zarr.core.Array(self.store, path='model_' + str(i))[:,:,:,:]), dtype=torch.float32)
            pressure = torch.tensor(np.array(zarr.core.Array(self.store, path='pressure_' + str(i))[:,:,:,1:]), dtype=torch.float32)
            ...
            if self.normalize:
                ...
                vorticity -= 1
                pressure -= pressure.min(); pressure /= pressure.max() / 2
                pressure -= 1
            ...
            if self.clip is not None:
                ...
                vorticity[vorticity < -self.clip] = -self.clip
                pressure[pressure > self.clip] = self.clip
                pressure[pressure < -self.clip] = -self.clip

                if self.normalize:  # renormalize
                    vorticity = vorticity / torch.max(torch.abs(vorticity))
                    pressure = pressure / torch.max(torch.abs(pressure))
            ...
            pressure = pressure.view(1, nx, ny, nz, nt)

            # Stack output along channel dimension
            y = torch.cat((
                vorticity,
                pressure
                ),
...
class WaterlilyParallel(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cache is not None and filename in self.cache:
            ...
            x = torch.tensor(np.array(zarr.core.Array(self.store, path='model_' + str(i))[:,self.yStart:self.yEnd,:,:]), dtype=torch.float32)
            pressure = torch.tensor(np.array(zarr.core.Array(self.store, path='pressure_' + str(i))[:,self.yStart:self.yEnd,:,1:]), dtype=torch.float32)
